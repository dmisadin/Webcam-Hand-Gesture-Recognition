...
def load_models(opt):
    ...
    opt.pretrain_modality = opt.modality_det
    ...
    opt.pretrain_modality = opt.modality_clf
...
def main(): 
    ...
    ## Get list of videos to test (CHANGE MODALITY HERE FOR DEPTH/RGB)
    if opt.dataset == 'egogesture':
        subject_list = ['Subject{:02d}'.format(i) for i in [2, 9, 11, 14, 18, 19, 28, 31, 41, 47]]
        test_paths = []
        for subject in subject_list:
            #for x in glob.glob(os.path.join(opt.video_path, subject, '*/*/depth*')):
            for x in glob.glob(os.path.join(opt.video_path, subject, '*/*/rgb*')):
                test_paths.append(x)
    elif opt.dataset == 'nvgesture':
        df = pd.read_csv(os.path.join(opt.root_path ,opt.video_path, 'nvgesture_test_correct_cvpr2016_v2.lst'), delimiter=' ', header=None)
        test_paths = []
    ...
    for path in test_paths[:]:
        if opt.dataset == 'egogesture':
            ...
            opt.whole_path = os.path.join("./", *path.rsplit(os.sep, 5)[2:]).replace('\\', '/')
        print("Whole path:  " + opt.whole_path)
        videoidx += 1
        active_index = 0
        passive_count = 0
        active = False
        ...
        for i, (inputs, targets) in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                #print(inputs.size(1))
                if opt.modality_det == 'RGB' and inputs.size(1) == 3:
